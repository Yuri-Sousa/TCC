...
using System.IO;
public class bldwwpbaseobjects_awwp_impactmetadata : GxBaseBuilder
{
   string cs_path = "." ;
   public bldwwpbaseobjects_awwp_impactmetadata( ) : base()
   {
   }
   ...
   public override int AfterCompile( )
   {
      ...
      if ( ! File.Exists(@"bin\wwpbaseobjects.AWWP_IMPACTMETADATA.exe.config") || checkTime(@"bin\wwpbaseobjects.AWWP_IMPACTMETADATA.exe.config",cs_path + @"\WWPBASEOBJECTS\AWWP_IMPACTMETADATA.exe.config") )
      {
         File.Copy( cs_path + @"\WWPBASEOBJECTS\AWWP_IMPACTMETADATA.exe.config", @"bin\wwpbaseobjects.AWWP_IMPACTMETADATA.exe.config", true);
      }
      if ( ! File.Exists(@"bin\WCFClient.config") || checkTime(@"bin\WCFClient.config",cs_path + @"\WCFClient.config") )
      {
         File.Copy( cs_path + @"\WCFClient.config", @"bin\WCFClient.config", true);
      }
      return ErrCode ;
   }
   ...
   {
      bldwwpbaseobjects_awwp_impactmetadata x = new bldwwpbaseobjects_awwp_impactmetadata() ;
      x.SetMainSourceFile( "wwpbaseobjects\\awwp_impactmetadata.cs");
